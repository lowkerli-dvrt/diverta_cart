    ...
    $.ajax({
        ...
        success: function(data) {
            setTimeout(function() {
                ...
                spanCartTotals.html(
                    data["cart"]["numElements"] + " item(s), " +
                    accounting.formatMoney(totalNoShipping)
                );
            }, uiDelay);
        },
        failure: function(err) {
            setTimeout(function() {
                ...
                ).attr("title", "There was a problem while getting your cart. Refreshing<br>(<span class='kbd'>F5</span>) may help."
                ).tooltip({html: true});
            }, uiDelay);
        }
    });
...
$(document).ready(function() {
    ...
    $('span.tooltip-price').each(function(_, v) {
        ...
        $(parsedTitle).filter('span.price').each(function(_, v) {
            var $this_title = $(this);
            var price = parseInt($this_title.text());
            $this_title.html(accounting.formatMoney(price));
        });
        var newTitle = parsedTitle.map(function(i) {
